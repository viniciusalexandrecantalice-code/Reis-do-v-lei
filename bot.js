...
function HBInit(config) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            page.on('pageerror', err => console.error("Erro no navegador:", err.message));
            
            await page.goto('https://www.haxball.com/headless', { waitUntil: 'networkidle0' });

            const room = {};

            await page.exposeFunction('hb_onPlayerJoin', (player) => { if (room.onPlayerJoin) room.onPlayerJoin(player); });
            await page.exposeFunction('hb_onPlayerLeave', (player) => { if (room.onPlayerLeave) room.onPlayerLeave(player); });
            await page.exposeFunction('hb_onPlayerChat', (player, message) => { if (room.onPlayerChat) return room.onPlayerChat(player, message); });
            await page.exposeFunction('hb_onPlayerTeamChange', (player) => { if (room.onPlayerTeamChange) room.onPlayerTeamChange(player); });
            await page.exposeFunction('hb_onPlayerBallKick', (player) => { if (room.onPlayerBallKick) room.onPlayerBallKick(player); });
            await page.exposeFunction('hb_onTeamGoal', (team) => { if (room.onTeamGoal) room.onTeamGoal(team); });
            await page.exposeFunction('hb_onGameStart', () => { if (room.onGameStart) room.onGameStart(); });
            await page.exposeFunction('hb_onGameStop', () => { if (room.onGameStop) room.onGameStop(); });
            await page.exposeFunction('hb_onGameTick', () => { if (room.onGameTick) room.onGameTick(); });
            
            await page.evaluate(cfg => {
                window._hbRoom = window.HBInit(cfg);
                window._hbRoom.onPlayerJoin = p => window.hb_onPlayerJoin(p);
                window._hbRoom.onPlayerLeave = p => window.hb_onPlayerLeave(p);
                window._hbRoom.onPlayerChat = (p, m) => window.hb_onPlayerChat(p, m);
                window._hbRoom.onPlayerTeamChange = p => window.hb_onPlayerTeamChange(p);
                window._hbRoom.onPlayerBallKick = p => window.hb_onPlayerBallKick(p);
                window._hbRoom.onTeamGoal = t => window.hb_onTeamGoal(t);
                window._hbRoom.onGameStart = () => window.hb_onGameStart();
                window._hbRoom.onGameStop = () => window.hb_onGameStop();
                window._hbRoom.onGameTick = () => window.hb_onGameTick();
            }, config);

            room.sendAnnouncement = (msg, targetId, color, style, sound) => page.evaluate(({ msg, targetId, color, style, sound }) => { if (window._hbRoom) window._hbRoom.sendAnnouncement(msg, targetId, color, style, sound); }, { msg, targetId, color, style, sound });
            room.getPlayerList = () => page.evaluate(() => window._hbRoom ? window._hbRoom.getPlayerList() : []);
            room.getScores = () => page.evaluate(() => window._hbRoom ? window._hbRoom.getScores() : null);
            room.startGame = () => page.evaluate(() => { if (window._hbRoom) window._hbRoom.startGame(); });
            room.stopGame = () => page.evaluate(() => { if (window._hbRoom) window._hbRoom.stopGame(); });
            room.setCustomStadium = (stadium) => page.evaluate(s => { if (window._hbRoom) window._hbRoom.setCustomStadium(s); }, stadium);
            room.setScoreLimit = (limit) => page.evaluate(l => { if (window._hbRoom) window._hbRoom.setScoreLimit(l); }, limit);
            room.setTimeLimit = (limit) => page.evaluate(l => { if (window._hbRoom) window._hbRoom.setTimeLimit(l); }, limit);
            room.setTeamsLock = (lock) => page.evaluate(l => { if (window._hbRoom) window._hbRoom.setTeamsLock(l); }, lock);
            room.setTeamColors = (team, angle, textColor, colors) => page.evaluate(({ team, angle, textColor, colors }) => { if (window._hbRoom) window._hbRoom.setTeamColors(team, angle, textColor, colors); }, { team, angle, textColor, colors });
            room.kickPlayer = (id, reason, ban) => page.evaluate(({ id, reason, ban }) => { if (window._hbRoom) window._hbRoom.kickPlayer(id, reason, ban); }, { id, reason, ban });
            room.clearBans = () => page.evaluate(() => { if (window._hbRoom) window._hbRoom.clearBans(); });
            room.setPlayerAdmin = (id, admin) => page.evaluate(({ id, admin }) => { if (window._hbRoom) window._hbRoom.setPlayerAdmin(id, admin); }, { id, admin });
            room.setPlayerTeam = (id, team) => page.evaluate(({ id, team }) => { if (window._hbRoom) window._hbRoom.setPlayerTeam(id, team); }, { id, team });
            room.setPlayerAvatar = (id, avatar) => page.evaluate(({ id, avatar }) => { if (window._hbRoom) window._hbRoom.setPlayerAvatar(id, avatar); }, { id, avatar });
            room.setPlayerName = (id, name) => page.evaluate(({ id, name }) => { if (window._hbRoom) window._hbRoom.setPlayerName(id, name); }, { id, name });
            room.pauseGame = (pause) => page.evaluate(p => { if (window._hbRoom) window._hbRoom.pauseGame(p); }, pause);

            resolve(room);
        } catch (err) {
            console.error("Erro ao iniciar o Puppeteer/Haxball:", err);
            reject(err);
        }
    });
}
...
// ===================================================
// A PARTIR DAQUI É O CÓDIGO DO SEU BOT
// ===================================================

HBInit({
  roomName: "𝟕 Rayzers 2v2",
  maxPlayers: 12,
  public: true, // DEIXEI TRUE PARA APARECER NA LISTA DO HAXBALL
  noPlayer: true,
  token: "thr1.AAAAAGptMEVKIIDXaUXkeA.Z5juo7VoqAc" // COLOQUE SEU TOKEN AQUI
}).then(room => {

  // Evento para mostrar o link da sala no console do servidor
